...
test_that("Settings leave-one-out can be constructed and hyperparameters added", {
  ...
  weights_settings_loo = prepare_settings_leave_one_out_characterization(lr_network_human,signaling_network_human, gr_network_human, source_weights_df)
...
test_that("Settings one-vs-one can be constructed and hyperparameters added", {
  ...
  weights_settings_ovo = prepare_settings_one_vs_one_characterization(lr_network_human,signaling_network_human, gr_network_human)
  ...
  weights_settings_ovo = prepare_settings_one_vs_one_characterization(lr_network_human,signaling_network_human, gr_network_human, lr_network_separate = TRUE)
  expect_type(weights_settings_ovo, "list")
  expect_type(weights_settings_ovo[[2]], "list")
  expect_type(weights_settings_ovo[[2]]$source_weights, "double")
  expect_type(weights_settings_ovo[[2]]$model_name, "character")
...
test_that("Leave-one-in models can be evaluated and results of this further processed", {
  ...
  output_characterization_random = lapply(weights_settings_loi[2:3],evaluate_random_model, lr_network_human,signaling_network_human, gr_network_human,settings,calculate_popularity_bias_target_prediction = FALSE, calculate_popularity_bias_ligand_prediction = FALSE, ncitations, cutoff_method = "quantile")
  expect_type(output_characterization_random, "list")


  output_characterization_application = lapply(weights_settings_loi[1:2],evaluate_model_application,lr_network_human,signaling_network_human, gr_network_human,settings[1:3], cutoff_method = "quantile") # make discrete ligand-target matrix via the quantile-method!
...
test_that("Influence individual data source on ligand-target scores can be assessed", {
  ...
  output = assess_influence_source("ontogenet_coarse", lr_network_human,signaling_network_human, gr_network_human, source_weights_df, ligands,lr_sig_hub = 0.25,gr_hub = 0.5,ltf_cutoff = 0,algorithm = "PPR",damping_factor = 0.8,correct_topology = TRUE)
  ...
  output = assess_influence_source("ontogenet_coarse", lr_network_human,signaling_network_human, gr_network_human, source_weights_df, ligands, rankings = TRUE, lr_sig_hub = 0.25,gr_hub = 0.5,ltf_cutoff = 0,algorithm = "PPR",damping_factor = 0.8,correct_topology = TRUE)
  expect_type(output, "list")
  ...
  output = assess_influence_source("ontogenet_coarse", lr_network_human,signaling_network_human, gr_network_human, source_weights_df, ligands, rankings = TRUE, matrix_output = TRUE, lr_sig_hub = 0.25,gr_hub = 0.5,ltf_cutoff = 0,algorithm = "PPR",damping_factor = 0.8,correct_topology = TRUE)
  expect_type(output, "list")
  expect_equal(output[[1]]$model %>% is.matrix(), TRUE)

  output = assess_influence_source("ontogenet_coarse", lr_network_human,signaling_network_human, gr_network_human, source_weights_df, ligands, rankings = FALSE,matrix_output = TRUE, lr_sig_hub = 0.25,gr_hub = 0.5,ltf_cutoff = 0,algorithm = "PPR",damping_factor = 0.8,correct_topology = TRUE)
  expect_type(output, "list")
  expect_equal(output[[1]]$model %>% is.matrix(), TRUE)
