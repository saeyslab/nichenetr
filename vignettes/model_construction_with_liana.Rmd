...
print_intersections <- function(liana_db, nichenet_db, organism = "human", type = "ligands"){
    n_before <- length(intersect(liana_db, nichenet_db))
    n_after <- length(intersect(convert_alias_to_symbols(unique(liana_db), organism, verbose = FALSE), nichenet_db))
    
    cat(n_before, " (", round(n_before / length(unique(liana_db)), 4)*100, "%) of ", type, " are in the NicheNet network.", sep = "")
    
    if (n_after-n_before > 0) {
    cat("\nAfter conversion, ", (n_after - n_before), " are added.", sep = "")
    }
}


lapply (show_resources()[-1], function(resource) {
  db <- select_resource(resource)[[1]] %>% decomplexify()
  # ligands <- unique(db$source_genesymbol)
  # targets <- unique(db$target_genesymbol)
  # cat(resource)
  # cat("\nThere are ", length(ligands), " ligands and ", length(targets), " receptors.\n", sep = "")
  # 
  ...
  # if (resource != "MouseConsensus"){
  #   print_intersections(db$source_genesymbol, lr_network_human$from, type = "ligands")
  #   cat("\n")
  #   print_intersections(db$target_genesymbol, sig_network_human$from, type = "receptors")
  #   
  # } else {
  #   print_intersections(db$source_genesymbol, lr_network_mouse$from, type = "ligands", organism = "mouse")
  #   cat("\n")
  #   print_intersections(db$target_genesymbol, sig_network_mouse$from, type = "receptors", organism = "mouse")
  #   
  # }
...
lr_network_liana <- select_resource("MouseConsensus")[[1]]
...
Run NicheNet with the wrapper function.
